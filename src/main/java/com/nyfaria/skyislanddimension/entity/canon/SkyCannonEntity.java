...
public class SkyCannonEntity extends Entity {
    private static final DataParameter<Integer> DATA_ID_HURT = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.INT);
    private static final DataParameter<Integer> DATA_ID_HURTDIR = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.INT);
    private static final DataParameter<Float> DATA_ID_DAMAGE = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> DATA_ID_TYPE = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.INT);
    private static final DataParameter<Boolean> DATA_ID_PADDLE_LEFT = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> DATA_ID_PADDLE_RIGHT = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> DATA_ID_BUBBLE_TIME = EntityDataManager.defineId(SkyCannonEntity.class, DataSerializers.INT);
    ...
    private SkyCannonEntity.Status status;
    private SkyCannonEntity.Status oldStatus;
    ...
    public SkyCannonEntity(EntityType<? extends SkyCannonEntity> p_i50129_1_, World p_i50129_2_) {
        super(p_i50129_1_, p_i50129_2_);
        this.blocksBuilding = true;
    }

    public SkyCannonEntity(World p_i1705_1_, double p_i1705_2_, double p_i1705_4_, double p_i1705_6_) {
        this(EntityInit.SKY_CANNON.get(), p_i1705_1_);
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_ID_TYPE, SkyCannonEntity.Type.OAK.ordinal());
    ...
    public void push(Entity pEntity) {
        if (pEntity instanceof SkyCannonEntity) {
    ...
    public Item getDropItem() {
        return ItemInit.CANNON_ITEM.get();
    }
    ...
    public void tick() {
        ...
        if (this.status != SkyCannonEntity.Status.UNDER_WATER && this.status != SkyCannonEntity.Status.UNDER_FLOWING_WATER) {
     ...
     */
    private SkyCannonEntity.Status getStatus() {
        SkyCannonEntity.Status boatentity$status = this.isUnderwater();
        if (boatentity$status != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return boatentity$status;
        } else if (this.checkInWater()) {
            return SkyCannonEntity.Status.IN_WATER;
        } else {
            float f = this.getGroundFriction();
            if (f > 0.0F) {
                this.landFriction = f;
                return SkyCannonEntity.Status.ON_LAND;
            } else {
                return SkyCannonEntity.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private SkyCannonEntity.Status isUnderwater() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
                ...
                for (int i2 = i1; i2 < j1; ++i2) {
                    ...
                    if (fluidstate.is(FluidTags.WATER) && d0 < (double) ((float) blockpos$mutable.getY() + fluidstate.getHeight(this.level, blockpos$mutable))) {
                        if (!fluidstate.isSource()) {
                            return SkyCannonEntity.Status.UNDER_FLOWING_WATER;
                        }
        ...
        return flag ? SkyCannonEntity.Status.UNDER_WATER : null;
    }
    ...
    private void floatBoat() {
        ...
        if (this.oldStatus == SkyCannonEntity.Status.IN_AIR && this.status != SkyCannonEntity.Status.IN_AIR && this.status != SkyCannonEntity.Status.ON_LAND) {
            this.waterLevel = this.getY(1.0D);
            this.setPos(this.getX(), (double) (this.getWaterLevelAbove() - this.getBbHeight()) + 0.101D, this.getZ());
            this.setDeltaMovement(this.getDeltaMovement().multiply(1.0D, 0.0D, 1.0D));
            this.lastYd = 0.0D;
            this.status = SkyCannonEntity.Status.IN_WATER;
        } else {
            if (this.status == SkyCannonEntity.Status.IN_WATER) {
                d2 = (this.waterLevel - this.getY()) / (double) this.getBbHeight();
                this.invFriction = 0.9F;
            } else if (this.status == SkyCannonEntity.Status.UNDER_FLOWING_WATER) {
                d1 = -7.0E-4D;
                this.invFriction = 0.9F;
            } else if (this.status == SkyCannonEntity.Status.UNDER_WATER) {
                d2 = (double) 0.01F;
                this.invFriction = 0.45F;
            } else if (this.status == SkyCannonEntity.Status.IN_AIR) {
                this.invFriction = 0.9F;
            } else if (this.status == SkyCannonEntity.Status.ON_LAND) {
    ...
    protected void readAdditionalSaveData(CompoundNBT pCompound) {
        if (pCompound.contains("Type", 8)) {
            this.setType(SkyCannonEntity.Type.byName(pCompound.getString("Type")));
        }
        ...
        if (!this.isPassenger()) {
            if (pOnGround) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != SkyCannonEntity.Status.ON_LAND) {
    ...
    public void setType(SkyCannonEntity.Type pBoatType) {
        this.entityData.set(DATA_ID_TYPE, pBoatType.ordinal());
    }

    public SkyCannonEntity.Type getBoatType() {
        return SkyCannonEntity.Type.byId(this.entityData.get(DATA_ID_TYPE));
    }
    ...
    public boolean isUnderWater() {
        return this.status == SkyCannonEntity.Status.UNDER_WATER || this.status == SkyCannonEntity.Status.UNDER_FLOWING_WATER;
    }
    ...
    public static enum Type {
         ...
         */
        public static SkyCannonEntity.Type byId(int pId) {
            SkyCannonEntity.Type[] aboatentity$type = values();
        ...
        public static SkyCannonEntity.Type byName(String pName) {
            SkyCannonEntity.Type[] aboatentity$type = values();
