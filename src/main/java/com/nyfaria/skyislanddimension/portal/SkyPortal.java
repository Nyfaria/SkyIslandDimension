package com.nyfaria.skyislanddimension.portal;
...
public class SkyPortal implements ITeleporter {
    ...
    public SkyPortal(boolean locked) {
        SkyPortal.locked = locked;
    }
    ...
    private static PortalInfo moveToSafeCoords(ServerWorld world, Entity entity) {
        ...
        boolean checkProgression = false;

        BlockPos pos = entity.blockPosition();
        if (isSafeAround(world, pos, entity, false)) {
    ...
    private static BlockPos findSafeCoords(ServerWorld world, int range, BlockPos pos, Entity entity) {
        ...
        for (int x = 0; x < attempts; x++) {
            for (int z = 0; z < attempts; z++) {
                ...
                if (isSafeAround(world, dPos, entity, false)) {
                    return dPos;
                }
            }
        }
    ...
    private static BlockPos makePortalAt(World world, BlockPos pos) {
        ...
        // grass all around it
        BlockState grass =  Blocks.AIR.defaultBlockState();

        world.setBlockAndUpdate(pos.below(), grass);
        world.setBlockAndUpdate(pos.west(), grass);
        world.setBlockAndUpdate(pos.west().below(), grass);
        world.setBlockAndUpdate(pos.west().above(), grass);
        world.setBlockAndUpdate(pos.west().above().above(), grass);
        world.setBlockAndUpdate(pos.west().above().above().above(), grass);
        world.setBlockAndUpdate(pos.above().above().above(), grass);
        world.setBlockAndUpdate(pos.east().above().above().above(), grass);

        world.setBlockAndUpdate(pos.east().below(), grass);
        world.setBlockAndUpdate(pos.east().east(), grass);
        world.setBlockAndUpdate(pos.east().east().below(), grass);
        world.setBlockAndUpdate(pos.east().east().above(), grass);
        world.setBlockAndUpdate(pos.east().east().above().above(), grass);
        world.setBlockAndUpdate(pos.east().east().above().above().above(), grass);
        ...
        world.setBlockAndUpdate(pos.above(), portal);
        world.setBlockAndUpdate(pos.above(2), portal);
        world.setBlockAndUpdate(pos.east(), portal);
        world.setBlockAndUpdate(pos.east().above(), portal);
        world.setBlockAndUpdate(pos.east().above(2), portal);
